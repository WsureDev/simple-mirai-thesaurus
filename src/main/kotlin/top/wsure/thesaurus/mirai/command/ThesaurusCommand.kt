...
object ThesaurusCommand : CompositeCommand(
    ...
    suspend fun CommandSender.del(args: MessageChain) {
        ThesaurusUtils.auth(this, { member ->
            val precise =
                THESAURUS_SERVICE.delThesaurus(member.group.id, Word(args.serializeToMiraiCode(), MessageType.PRECISE))
            val fuzzy =
                THESAURUS_SERVICE.delThesaurus(member.group.id, Word(args.serializeToMiraiCode(), MessageType.FUZZY))
            ThesaurusUtils.createGroupThesaurusCache(member.group.id)
            sendMessage("删除精确问${precise}个,删除模糊问:${fuzzy}个")
        }, {
            val precise =
                THESAURUS_SERVICE.delThesaurus(GLOBAL_TAG, Word(args.serializeToMiraiCode(), MessageType.PRECISE))
